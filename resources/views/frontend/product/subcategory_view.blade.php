...
@section('title')
Amymgroup.com | @if(session()->get('language') == 'french') {{$subCategoryInfo->subcategory_name_fr}} @else
                            {{$subCategoryInfo->subcategory_name_en}} @endif
@endsection
          ...
          <div class="col-12">
              ...
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                  ...
                  <li class="breadcrumb-item active" aria-current="page">@if(session()->get('language') == 'french') {{$subCategoryInfo->subcategory_name_en}} @else
                      {{$subCategoryInfo->subcategory_name_en}} @endif</li>
                </ol>
              </nav>
          </div>
      </div>
      ...
      <div class="row gx-10">
                                          ...
                                          <ul class="nav flex-column ms-3">
                                              ...
                                              @foreach ($subcategories as $subcategory)
                                                  @if(session()->get('language') == 'french') 
                                                      <span><a target="_blanc" href="#">{{$subcategory->subcategory_name_fr}}</a></span>
                                                  @else <span><a target="_blanc" href="#">{{$subcategory->subcategory_name_en}}</a></span>
                                                  @endif
                                              @endforeach
                                          </ul>
                                      </div>
          ...
          <section class="col-lg-9 col-md-12">
              ...
              <div class="card mb-4 bg-light border-0">
                  ...
                  <h2 class="mb-0 fs-1">
                      @if(session()->get('language') == 'french') {{$subCategoryInfo->subcategory_name_fr}} @else
                      {{$subCategoryInfo->subcategory_name_en}} @endif </h2>
                  </div>
              </div>
              <!-- list icon -->
          ...
          <div class="row g-4 row-cols-xl-4 row-cols-lg-3 row-cols-2 row-cols-md-2 mt-2">
              <!-- col -->
              @php
                  $subCatwiseProduct = App\Models\Product::where('subcategory_id',$subCategoryInfo->id)->orderBy('id','DESC')->get(); 
              @endphp
              @forelse($subCatwiseProduct as $product)
              <div class="col">
                  <!-- card -->
                          ...
                          @if(session()->get('language') == 'french') 
                            <div class="card-product-action">
                                  <a href="{{ url('product/details/'.$product->id.'/'.$product->product_slug_fr ) }}" 
                                    class="btn-action"> <i class="bi bi-eye" data-bs-toggle="tooltip" data-bs-html="true" title="Details du produits"></i></a>
                                  <a href="#!" class="btn-action" data-bs-toggle="tooltip" data-bs-html="true" title="Discutez du produit sur whatsapp">
                                     <i class="bi bi-whatsapp"></i></a>
                                  <a href="#!" class="btn-action" data-bs-toggle="tooltip" data-bs-html="true" title="Discutez du produit sur facebook">
                                      <i class="bi bi-facebook"></i></a>
                              </div>
                            @else 
                                <div class="card-product-action">
                                  <a href="{{ url('product/details/'.$product->id.'/'.$product->product_slug_en ) }}" 
                                    class="btn-action"> <i class="bi bi-eye" data-bs-toggle="tooltip" data-bs-html="true" title="Product details"></i></a>
                                  <a href="#!" class="btn-action" data-bs-toggle="tooltip" data-bs-html="true" title="Chat about the product on whatsap">
                                     <i class="bi bi-whatsapp"></i></a>
                                  <a href="#!" class="btn-action" data-bs-toggle="tooltip" data-bs-html="true" title="Chat about the product on facebook">
                                      <i class="bi bi-facebook"></i></a>
                                </div>
                            @endif
