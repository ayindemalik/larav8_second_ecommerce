...
class UserCargoController extends Controller
{
    ...
    public function manageCargoTypes(){
        ...
        return view('dashboard.useraccount.cargo_operation.cargo_type.manage_all', compact('wCargoTypes', 'fCargoTypes'));
    }

    // New
    public function addNewCargoType(){
        $countries = Country::latest()->get();
        return view('dashboard.useraccount.cargo_operation.cargo_type.category_type_add', compact('countries'));
    }
    ...
    public function storeCategoryType(Request $request){
        ...
        if($insert_id > 0){
            ...
            return redirect()->route('user-manage-cargo-types')->with($notification);
        }
        else {
            $notification = array(
    ...
    public function loadForm($action, $val){
        if($val != 0){
            ...
            // dd($data);
            return view('dashboard.useraccount.cargo_operation.cargo_type.cargo_type_form', compact('data'));
        }
        else
            return view('dashboard.useraccount.cargo_operation.cargo_type.cargo_type_form');
    }
    ...
    public function editCategoryType($id){
        ...
        $countries = Country::latest()->get();
        $cargoTypData = CargoTypeList::findOrFail($id);
        // dd($cargoTypData);
        return view('dashboard.useraccount.cargo_operation.cargo_type.category_type_edit', compact('countries', 'cargoTypData'));
    }
    public function storeCategoryTypeUpdate(Request $request){
        // dd($request);
        $upd = 0;
        if($request->type === "W"){
            $existData = CargoTypeList::where('name', $request->crg_typ_name)
            ...
            if(is_array($existData) && count($existData)>0){ // Alreaddy exist
                ...
                $upd = CargoTypeList::findOrFail($request->form_id)
                    ->update([
                        'type' => $request->type,
                        'country_id' => $request->country,
                        'name' => $request->crg_typ_name,
        ...
        if($upd == true){
            ...
            $notification = array(
                'message' => 'Error or no changes! Cargo type was not updated ',
                'alert-type' => 'error'
                );
            return redirect()->back()->with($notification);
        }
    ...
    public function viewShipmentList(){
        ...
        return view('dashboard.useraccount.cargo_operation.shipment_transactions.trans_index', compact('cargo_list'));
    } 
    ...
    public function addNewShipment(){
        ...
        $wCargoTypes = CargoTypeList::where('type', 'W')->latest()->get();
        $fCargoTypes = CargoTypeList::where('type', 'F')->latest()->get();
        return view('dashboard.useraccount.cargo_operation.shipment_transactions.add_shipment', compact('wCargoTypes', 'fCargoTypes', 'countries', 'refcode'));
    } 
        ...
        if($cid > 0){
            ...
            if($request->shipType == 'W'){
                ...
                if($total_items == count($request->cargo_type_id)){
                    ...
                    return redirect()->route('user-view-shipment-list');
                }
                else{
    ...
    public function editShipment($id){
        ...
        $wCargoTypes = CargoTypeList::where('type', 'W')->latest()->get();
        $fCargoTypes = CargoTypeList::where('type', 'F')->latest()->get();
        return view('dashboard.useraccount.cargo_operation.shipment_transactions.edit_shipment', 
    ...
    public function viewShipmentDetails($id,$ref_code){
        ...
        $wCargoTypes = CargoTypeList::where('type', 'W')->latest()->get();
        $fCargoTypes = CargoTypeList::where('type', 'F')->latest()->get();
        return view('dashboard.user.cargo_operation.shipment_transactions.trans_view_details', 
    ...
    public function deleteShipment($cargo_id){
        ...
        if(!empty(CargoList::where('id',$cargo_id)->get())){
            ...
            if($delOp > 0){
                ...
                return redirect()->route('user-view-shipment-list')->with($notification);
            }
        }
    }
    ...
    public function printShipmentPdf($id, $code){
        ...
        $pdf = PDF::setOptions(['isHtml5ParserEnabled'=>true,'isRemoteEnabled'=>true])
                    ->loadView('dashboard.useraccount.cargo_operation.shipment_transactions.print_shipment_pdf', 
